...
- `itineraries`: local Ollama itinerary generation from trip inputs and attractions.
...
- Ollama provides local LLM itinerary generation for Sprint 2.
- Future real-time Travelpayouts Search API requires marker, signature, host, and server-side request handling.
- Future hosted LLM providers can be added through the existing itinerary service boundary.
